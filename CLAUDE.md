...
- **【待核实·暂不处理】下游 bug 汇报（2026-08-03）**：`_exhausted_result` 的 `best is not None` 分支疑似漏传 `ground_truth`（generator.py:207-214），或致 MISMATCH 耗尽 failed 样本缺 GT 字段、卡住 convert `--include-failed` 硬样本流程。**按用户指示仅记录、未验证未修复**（investigation-01 追加七）。
- **【待核实·暂不处理】下游 bug 汇报 ②（2026-08-03）**：merge/combine 均以 `sample_id` 为键，下游反馈同 id 记录被合并丢数据——案例：三 part 各 700 success+300 overturned，unique 后剩 2000、combine 后只剩 1000，疑似各批 sample_id 撞车 + 去重后赢静默覆盖。**按用户指示仅记录、未核查未修复**（investigation-01 追加八，与追加七建议一并排期）。
