...
- 测试基线：`python -m pytest tests/ -v`（全部 mock，不触真实 API，286 项指标测试 + 2 项 slow 冒烟，实测结果见 `doc/test-results.md`）。环境：`.venv`（uv 创建，依赖 aiohttp / pytest / pytest-asyncio，pip 源用清华镜像）。
...
- **下游 bug 汇报两条已修复（2026-08-04）**：① `_exhausted_result` best 分支漏传 `ground_truth` 属实——generator 补传一行根治 + convert 增加 original_sample GT 回退救旧数据（救回数计 `failed_gt_recovered`）；② combine 丢数据现象属实、归因修正——坑在 **sample_id 只保证单 run 内唯一**（缺 id 按位置补 sample_{i}，跨批整批撞车），combine 去重键改 **(输入路径, sample_id)**（路径内后赢不变，跨路径同 id 全保留，撞车计 `cross_path_id_collisions`），merge 加重复 id 防御计数（均不改 join 语义）。防误解清单新增 §5.22（id 唯一性范围 + GT 双防线）。测试 261（+8）。
- **polish 润色/修复工具（2026-08-05）**：`cotbuilder/polish.py`（`python -m cotbuilder.polish`，骨架镜像 judge.py）——**polish 模式**：只给模型看原始问题+CoT（不看图不给 GT），规则化+few-shot prompt 收敛纠结冗余，输出 {"cot","answer"} 与原 predicted_json 做 Matcher STRICT 一致性校验（**比的是同一次调用的 answer 不是 GT**），不一致 → applied=false 一次定音不重试（重试走 failed 文件+新 output+combine 循环）；**repair 模式**：连问题+CoT+GT 丢给模型修，**不做任何验证**（用户拍板）。标签块 `polish_result`（不改原字段），applied → success 文件；convert 自动优先 polished_cot/polished_answer（thinking/json 模式），run → polish → convert 直接串。convert 的 human_text/reasoning_text/gt_from_original_sample 提为公开 helper 供复用。防误解清单 §5.23。测试 286（+25）。
- **操作文档面向新手补全（2026-08-05）**：workflow.md 新增「三分钟版」最短路径（cli→convert 两步必做，judge/polish 可选增强）、概念速览表（success=golden answer+CoT、train.json=交付物）、golden answer 提取 recipe（`--gpt-mode json`，已确认现有功能覆盖无需新开发）、常见状况表（中断重跑续跑/403 处置/耗时估算/试跑参数）；README Quick Start 改为最短闭环+可选增强分层。
