...
- 测试基线：`python -m pytest tests/ -v`（全部 mock，不触真实 API，242 项指标测试 + 2 项 slow 冒烟，实测结果见 `doc/test-results.md`）。环境：`.venv`（uv 创建，依赖 aiohttp / pytest / pytest-asyncio，pip 源用清华镜像）。
...
- **merge + convert 离线工具（2026-08-03）**：`cotbuilder/merge.py`（`python -m cotbuilder.merge`）把 judge 结果并回原 run——翻转+搬移+标签（`judge_result` 键存在=被判过，原字段不动），支持反复 judge 循环；`cotbuilder/convert.py`（`python -m cotbuilder.convert`）转 ShareGPT 训练数据——gpt 轮默认 `<thinking>` 包裹+JSON（推理链回收 reasoning_content → cot_response 剥离，design.md §5.21），可切 raw/json；容器默认 JSON 数组（对齐 13 万条老数据微调输入）可切 jsonl；human 末尾按实际是否含推理自动加 `/think` `/no_think` 软开关标志；`--strip-fences` 删 raw 模式 ```json 围栏；`--mix` 无 CoT ShareGPT 按比例混入（自动去 thinking 段 + 加 `/no_think`）。两者纯离线只读（§5.20）。**全部 JSON 格式总览见 `doc/formats.md`**。测试 242（+25）。
