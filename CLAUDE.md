...
- 测试基线：`python -m pytest tests/ -v`（全部 mock，不触真实 API，261 项指标测试 + 2 项 slow 冒烟，实测结果见 `doc/test-results.md`）。环境：`.venv`（uv 创建，依赖 aiohttp / pytest / pytest-asyncio，pip 源用清华镜像）。
